...
The general syntax of filter is: `filter(dataset, condition)`. In case it gets added in a pipeline, you will only see the condition argument, as the dataset is piped into the function.  
...
Another sometimes useful function is `near()`.  
The sample code will select all rows where `sleep_total` is nearly 17. You have to specify a tolerance `tol` to indicate how far the values can be. You can add a specific number: `filter(near(sleep_total, 17, tol = 0.5))` for instance will return any rows where sleeptotal is between 16.5 and 17.5, or you can add a formula. The sample code will return all rows that are within one standard deviation of 17.
...
`filter(xor(condition1,ondition)` will return all rows where only one of the conditions is met, and not when both conditions are met.
...
The `dplyr` package has a few powerful variants to filter multiple columns:  
+ `Filter_all` to filter across all columns
+ `Filter_if` and `filter_at` to filter across a few specified columns
  
In these cases, there is a general syntax: first you specify which columns, then you mention the condition based on which to filter. In many cases you will need a `.` operator within the condition which refers to the values we are looking at.
