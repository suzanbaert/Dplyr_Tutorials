...
Similarly, when parsing an R object to JSON with the `jsonlite::toJSON()` function, by defaultyou get the non-indented form, but by adding the argument `pretty = TRUE` (as will be done throughout this tutorial) it will print its indented form.
...
If it needs to be JSON object, you need to first get to a list. If you do not have many elements you could simply build a list manually and refer to each element in the datatable, but with many columns that's a lot of manual labour. There are two others ways: iterate over every row of your data and call `as.list` on that row:   

```{r}
toJSON(as.list(new_data[1, ]), auto_unbox = TRUE, pretty = TRUE)
```

or first tranpose your entire datatable to an inside-out-list, and send each element of the list seperately. 
...
Imagine your dataframe already covers a part of the required json string, but it needs some additional info. One way is to embed your data inside a list. You can embed one or more dataframes, and those can be identified upfront or inline.
...
Below is an example of two embedded dataframes, one of which is made at the time of the list call. 
...
Those are still somewhat optimistic cases though. You might have to provide the meta info per observation rather than for the bulk
