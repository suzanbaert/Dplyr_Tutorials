...
Note: as per previous blog posts, I will present everything in the form of a pipe. In some of these cases this might not be necessary and it would be just as easy to write it as a single function, but as I want to present options that you can use in your pipes, all below examples will be piped.
...
The easiest way to know how observations you have for a specific variable, is to use `count()`. By adding the `sort = TRUE` argument immediately returns a sorted table with descending number of observations:
...
If you're only interested in counting the total number of cases for a dataframe, you could use `tally()`, which behaves simarly to `nrow()`. The latter is shorter so I would still prefer that one.

You can't provide a group with `tally()`, it only works to count the overall number of observations. In fact, as is described in the `dplyr` documentation, `count()` is a short-hand for `group_by()` and `tally()`. 
...
More interesting is the `add_tally()` function which automatically adds a column with the overall number of observations. This is will be the same as `mutate(n = n())`.
...
Even more interesting is `add_count()` which takes a variable as argument, and adds a column which the number of observations. This saves the combination of grouping and mutating.
...
+ `n_distinct(var)` - gives the numbers of unique values or `var` present
+ `sum(var)`, 
+ `mean(var)`, `median(var)`, `sd(var)`, `IQR(var)`, ... 
+ 


any others I want to mention?
...
One of the downsides of the aggregate summarise functions is that you do not require a new column title. It therefore might not always be clear which this new value is. Luckily thanks to similar `rename_*()` functions, it only takes one line extra to rename them all:
...
It's useful if your summary tables are arranged. This is when the `arrange()` function comes in. The default format for numeric variables is to sort ascending, but you can add the `desc()` function in your call to change the default. For string variables, it will sort alphabetically.
...
You can use this function without having a `summarise()` call upfront, in which you most likely will have to spell out a variable to consider: 
